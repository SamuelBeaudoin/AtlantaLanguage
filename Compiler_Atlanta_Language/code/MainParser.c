...
atlanta_int errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern atlanta_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern atlanta_int line; /* source code line number - defined in scanner.c */
...
extern atlanta_nul startParser(void);
extern atlanta_int startScanner(BufferPointer sc_buf);

static atlanta_nul printParserError(char* fmt, ...);
static atlanta_nul displayParser(BufferPointer ptrBuffer);
static atlanta_lng getParserFileSize(char* fname);
static atlanta_nul callGarbageCollector(void);
...
atlanta_nul printParserError(atlanta_chr* fmt, ...) {
...
atlanta_lng getParserFileSize(atlanta_chr* fname) {
	FILE* input;
	atlanta_lng flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
atlanta_nul displayParser(Buffer* ptrBuffer) {
...
atlanta_nul callGarbageCollector(sofia_nul) {
